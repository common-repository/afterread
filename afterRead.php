...
Plugin URI: http://blog.warll.com/2010/afteread/
...
/*  
	The afterRead core and all bundled 'suggestions' are released under the GPL:
	http://www.opensource.org/licenses/gpl-license.php
	Suggestions that did not come from a default afterRead install may or may not be gpl, it is the Suggestion author's choice.
...
function afterRead_settings_link($links) {
	if( function_exists( "admin_url" ) ) {
		$settings_link = '<a href="' . admin_url( 'options-general.php?page=afterRead_options' ) . '">' . __('Settings') . '</a>';
		array_unshift($links, $settings_link);
	}
	return $links;
}

$afterRead_plugin = plugin_basename(__FILE__);
add_filter("plugin_action_links_$afterRead_plugin", 'afterRead_settings_link' );
...
add_filter( "the_content", "afterRead_main"); 

//Runs afterRead_activation() when the plugin is activated
register_activation_hook(__FILE__, 'afterRead_activation');
function afterRead_activation(){
	...
	$afterRead_secure = mt_rand();
	add_option( 'afterRead_secure', $afterRead_secure, '', 'no');
	update_option( 'afterRead_secure', $afterRead_secure );
	add_option( 'afterRead_suggestion_name', 'default', '', 'yes');
	add_option( 'afterRead_custom_html', '<hr />Go to the main <a href="'.get_option('home').'/"> page </a><hr />', '', 'yes');
	//Because save_guggestion.php, the file the handles writing out the "add new suggestion" admin option, cannot call the database we have to keep the key someplace where it can access it. It is saved in a php file so that a stranger cannot call the key file as they would be be able to if it was a .txt
	$afterRead_file_to_write = '<?php; function afterRead_print_key() {return "'. $afterRead_secure . '";}?>';
	$afterRead_name_to_write = dirname(__file__).'/afterRead_key_store.php';
	$afterRead_fh = @fopen($afterRead_name_to_write, 'w') or die("Can't open file");
	@fwrite($afterRead_fh, $afterRead_file_to_write);
	@fclose($afterRead_fh);
}
...
function afterRead_main($afterRead_content) {
	...
	//merge the wordpress post(afterRead_content) with the suggestion ($afterRead_sugestion) and the afterRead tag
...
function register_afterReadsettings() {
	register_setting( 'afterRead-settings-group-1', 'afterRead_suggestion_name' );
	register_setting( 'afterRead-settings-group-2', 'afterRead_custom_html' );
	register_setting( 'afterRead-settings-group-2', 'afterRead_option_ichi' );
	register_setting( 'afterRead-settings-group-2', 'afterRead_option_ni' );
	register_setting( 'afterRead-settings-group-2', 'afterRead_option_san' );
}
...
<div class="wrap">
	...
	<p>
		<form method="post" action="options.php">
			<?php settings_fields( 'afterRead-settings-group-1', 'afterRead' ); ?>
			<table class="form-table">
				<h3><?php _e('Suggestion selection', 'afterRead')?></h3>
				<tr valign="top">
					<th scope="row"><?php _e("Select Suggestion", 'afterRead') ?></th>
					<td>
			...
			<p class="submit">
				<input type="submit" class="button-primary" value="<?php _e('Save selection', 'afterRead') ?>" />
			</P>
		</form>
	</p>
	<p>
		<form method="post" action="options.php">
			<h3><?php _e('Suggestion options', 'afterRead')?></h3>
			<?php settings_fields( 'afterRead-settings-group-2', 'afterRead' ); ?>
			<?php _e('These options are used differently by each suggestion, many suggestions will not use them at all.', 'afterRead')?>
			<table class="form-table">
				<tr valign="top">
					<th scope="row"><?php _e('Custom HTML and Text', 'afterRead') ?></th>
					<td><input type="text" size="30" name="afterRead_custom_html" value="<?php echo htmlspecialchars(get_option('afterRead_custom_html'));?>" /></td>
				</tr>
				...
				<tr valign="top">
					<th scope="row"><?php _e('Option Ichi', 'afterRead') ?></th>
			...
			<p class="submit">
				<input type="submit" class="button-primary" value="<?php _e('Save Options', 'afterRead') ?>" />
			</p>
		</form>
	</p>
	<p>
		<h3><?php _e('Suggestion readout', 'afterRead')?></h3>
		<?php _e('The contents of the suggestion ', 'afterRead')?>"<?php echo get_option('afterRead_suggestion_name')?>" <br />
		<textarea  class="large-text code" rows="13" readonly><?php echo htmlspecialchars(file_get_contents(dirname(__FILE__).'/suggestions/'.get_option('afterRead_suggestion_name').'.php')) ?></textarea>
	</p>
		
	<p>	
		<h3><?php _e('Add Suggestion', 'afterRead')?></h3>
		<form method="post" action="<?php echo get_option('home').'/wp-content/plugins/afterread/save_suggestion.php'?>">
			<input type="hidden" name="afterRead_secure" value="<?php echo get_option('afterRead_secure')?>"/>
			<tr valign="top">
				<th scope="row"><?php _e('Name of suggestion to add:', 'afterRead') ?>
					<input type="text" size="15" name="afterRead_suggestion_new_name" value="" />
					<?php _e('The code for the suggestion goes below', 'afterRead') ?>
				</th>
			</tr>
			<textarea name="afterRead_suggestion_new_php" class="large-text code" rows="13" ></textarea>
			<?php _e('This will add a new suggestion file or rewrite an already existing file depending on the suggestion name given', 'afterRead') ?>
			<p><?php //The onlcick, settimeout is done to give the server time to proccess the request so that the new suggestion will show up in the drop down list without the need for a manual reload?>
				<input type="submit" class="button-primary" value="<?php _e('Save Suggestion', 'afterRead') ?>" onClick="setTimeout('location.reload(true)',250);"/>
			</p>
		</form>
	</p>
	
	<p>
		This plugin was developed by <a href="http://blog.warll.com">Warll Dressler</a>, if there is something you think afterRead needs please <a href="mailto:warll.dressler@gmail.com">email</a> me or leave a comment <a href="http://blog.warll.com/2010/afteread/">here</a>.
	</p>
</div>
<?php }
